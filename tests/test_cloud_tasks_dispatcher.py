...
def test_cloud_tasks_dispatcher_adds_oidc_token_when_configured(monkeypatch) -> None:
    get_settings.cache_clear()
    fake_client = FakeCloudTasksClient()
    fake_tasks_module = type(
        "FakeTasksModule",
    ...
    monkeypatch.setenv("WORKER_AUDIENCE", "https://worker-audience.example.com")
    monkeypatch.setattr(dispatcher_module, "get_cloud_tasks_client", lambda: fake_client)
    ...
    dispatcher = CloudTasksJobDispatcher()
    record = JobRecord(job_id="job-2", merchant_id="merchant-1", request=_request())

    dispatcher.dispatch(record)

    http_request = fake_client.created[0]["task"]["http_request"]
    assert http_request["oidc_token"] == {
        "service_account_email": "runner@project-1.iam.gserviceaccount.com",
        "audience": "https://worker-audience.example.com",
    }
    get_settings.cache_clear()
